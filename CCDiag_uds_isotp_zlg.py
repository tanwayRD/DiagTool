...
DTO_TX_ID = 0x795
DTO_RX_ID_PHYS = 0x715
DTO_RX_ID_FUNC = 0x7DF

EPS_TX_ID = 0x795
EPS_RX_ID_PHYS = 0x715

EPS4wd_TX_ID = 0x795
EPS4wd_RX_ID_PHYS = 0x715
...
class ZCAN_CCDiag(tk.Tk):
    ...
    def DeviceInit(self):
        ...
        self._isotpaddr_PHYS = isotp.Address(isotp.AddressingMode.Normal_11bits, txid=DTO_RX_ID_PHYS, rxid = DTO_TX_ID)
        self._isotpaddr_FUNC = isotp.Address(isotp.AddressingMode.Normal_11bits, txid=DTO_RX_ID_FUNC, rxid = DTO_TX_ID)
    ...
    def WidgetsInit(self):
        ...
        self.tabesc = self.gbDiag.add(self.frameesc, text = "   DTO   ")
        self.frameeps = tk.Frame(self.gbDiag)
        self.tabeps = self.gbDiag.add(self.frameeps, text = "   SCOPE   ")
        self.frameepb = tk.Frame(self.gbDiag)
        self.tabepb = self.gbDiag.add(self.frameepb, text = "   TEMPO A3  ")
    ...
    def DiagWidgetsInit(self):
        ...
        tk.Label(self.frameesc, text = "DTO软件路径:").grid(row = 8, column = 0)
        ...
        #diag test part################################################################
        self._v = IntVar()
        self.Radioesc = Radiobutton(self.frametest,text='DTO',variable=self._v,value=1) #TODO. need to add function for switching addressing
        self.Radioesc.grid(row=0, column=0, padx=3, pady=3)
        self.Radioeps = Radiobutton(self.frametest,text='SCOPE',variable=self._v,value=2)
        self.Radioeps.grid(row=0, column=1, padx=3, pady=3)
        self.Radioepb = Radiobutton(self.frametest,text='TEMPO A3',variable=self._v,value=3)
    ...
    def ChnInfoUpdate(self, is_open):
        ...
        if is_open:
            ...
            if cur_chn_info["is_canfd"] == True:
                ...
                self.cmbDataBaudrate.current(1)
...
###############################################################################
    ...
    def onTabChange(self, event):
        ...
        if event.widget.index("current") == 0: 
            ...
            print("INFO:Setting address DTO")
    ...
    def BtnReadSwVer_Click(self):
        ...
        resp = self.udsclient.read_data_by_identifier(0xF189)
        print(resp.data[2:])
...
###############################################################################
    ...
    def isotp_rcv(self):
        can_num = self._zcan.GetReceiveNum(self._can_handle, ZCAN_TYPE_CANFD)
        if can_num and not self._terminated:
            read_cnt = MAX_RCV_NUM if can_num >= MAX_RCV_NUM else can_num
            can_msgs, act_num = self._zcan.ReceiveFD(self._can_handle, read_cnt, MAX_RCV_NUM)
            list = []
            for i in range(can_msgs[0].frame.len):
                val = can_msgs[0].frame.data[i]
                list.append(val)
            #canfd len转dlc
            fdlen = can_msgs[0].frame.len
            if fdlen <= 8:
                fdlen = 8
            elif fdlen == 12:
                fdlen = 9
